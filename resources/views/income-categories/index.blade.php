...
@section('title', 'Quản lý Danh mục Thu nhập')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Quản lý Danh mục Thu nhập</h1>
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form id="categoryForm" class="p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4" id="modalTitle">Thêm danh mục thu nhập mới</h3>
                <div>
...
function openCreateModal() {
    ...
    document.getElementById('modalTitle').textContent = 'Thêm danh mục thu nhập mới';
...
function openEditModal(id, name) {
    ...
    document.getElementById('modalTitle').textContent = 'Sửa danh mục thu nhập';
...
document.getElementById('categoryForm').addEventListener('submit', function(e) {
    ...
    const url = currentCategoryId ? `/income-categories/${currentCategoryId}` : '/income-categories';
    ...
    .then(response => {
        if (!response.ok) {
            return response.json().then(data => {
                throw new Error(data.message || 'Không thể lưu danh mục thu nhập');
            });
        }
...
function deleteCategory(id) {
    if (confirm('Bạn có chắc muốn xóa danh mục thu nhập này?')) {
        fetch(`/income-categories/${id}`, {
            method: 'DELETE',
            headers: {
                'Accept': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        .then(response => {
            if (!response.ok) {
                return response.json().then(data => {
                    throw new Error(data.message || 'Không thể xóa danh mục thu nhập');
                });
            }
            return response.json();
        })
